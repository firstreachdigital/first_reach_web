...
function PortfolioInner() {
  ...
  const openEdit = (record) => {
    ...
    setVideoUrl(record.video || "");
    setMediaType(record.mediaType || "image");
    setModalOpen(true);
  };
  ...
  const handleVideoUpload = async ({ file, onSuccess, onError }) => {
    ...
    try {
      const { data } = await API.post("/portfolio/upload-video", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  const columns = [
    ...
    render: (v) => v === "video" ? <Tag color="magenta">AI Video</Tag> : <Tag>Image</Tag>,
   },
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Form.Item
            ...
            initialValue="Web design, Portfolio"
          >
          ...
          <Form.Item label="Media Type">
            <Radio.Group
              ...
              onChange={(e) => {setMediaType(e.target.value);
                if(e.target.value === "image") setVideoUrl("");
              }}
